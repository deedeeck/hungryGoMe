...
mysql_select_db("b12_13833487_123", $con);
  $region = mysql_real_escape_string($_POST['region']);
	...
	$food_type = mysql_real_escape_string($_POST['food_type']);
	
	if ( $region == "Unspecified" && $food_type == "Unspecified" && $rType == "Unspecified") 
	{$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE approve = '1'"); } 
	
	elseif ( $rType == "Unspecified" && $region == "Unspecified"){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE ft_name = '$food_type' AND approve = '1'"); }
	
	elseif ( $rType == "Unspecified" && $food_type == "Unspecified"){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE region = '$region' AND approve = '1'"); }
	
	elseif ( $food_type == "Unspecified" && $region == "Unspecified"){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE rt_name = '$food_type' AND approve = '1'"); }
	
	elseif ( $region == "Unspecified" ){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE ft_name = '$food_type' AND rt_name = '$rType' AND approve = '1'"); }
	
	elseif ( $food_type == "Unspecified" ){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE region = '$region' AND rt_name = '$rType' AND approve = '1'"); }
	
	elseif ( $rType == "Unspecified" ){
	$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE region = '$region' AND ft_name = '$food_type' AND approve = '1'"); }
	
	else {
		$result = mysql_query("SELECT DISTINCT stall_name FROM restaurant WHERE region = '$region'
								AND ft_name = '$food_type' AND rt_name = '$rType' AND approve = '1'");
	}
?>
    ...
    <p><a class="btn btn-primary btn-large" href="#">Read more »</a></p>
  	 ...
  	 <div class="row" style="text-align: center; padding: 0px;">
        <div style="display: inline-block; text-align: left; padding: 30px; background-color: #eeeeee; border-radius: 6px;">
			<h1 style="color: black; margin-top: 0;">Search Results for:</h1>
            
            <!-- search parameters + table of restaurants -->
			<?php
           		echo "<h3>" . $food_type . ", " . $region . ", " . $rType . "</h3>";
                ...
                while($row = mysql_fetch_array($result))
                {
                    ...
                    echo "<td><a href='searchToResturant.php?stall_name=".$row['stall_name']."'>". $row['stall_name'] . "</td>";
					echo "</tr>";
                }
                echo "</table>";
              ?>
