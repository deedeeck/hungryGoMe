...
if (!$con)
  ...
  die('Could not connect: ' . mysql_error());
	...
	//START TEST
		 $memberEmail = $_SESSION['MM_Username'];
	 
	mysql_select_db("b12_13833487_123", $con);
		
		if($_SESSION['AccessRights'] == "Member") {
			$sql="INSERT INTO restaurant (stall_name,address,region,ft_name,rt_name,telephone_number)	VALUES('$_POST[stall_name]','$_POST[address]','$_POST[region]','$_POST[ft_name]','$_POST[rt_name]','$_POST[telephone_number]')";
		} else {
			$sql="INSERT INTO restaurant (stall_name,address,region,ft_name,rt_name,telephone_number, approve)	VALUES('$_POST[stall_name]','$_POST[address]','$_POST[region]','$_POST[ft_name]','$_POST[rt_name]','$_POST[telephone_number]', 1)";
		}
		
		$abc="INSERT INTO submit_entry(m_email,postDate,r_stall_name,r_address) 
		VALUES('$memberEmail',NOW(),'$_POST[stall_name]','$_POST[address]')"; 
  	 ...
  	 <div class="row" style="text-align: center; padding: 50px;">
        <div style="display: inline-block; text-align: left; padding: 30px; background-color: #eeeeee; border-radius: 6px;">
            ...
            <?php
        		if(isset($_SESSION['MM_Username'])) {
			?>
            <h1 style="color: black; margin-top: 0;">Success!</h1>
            <p>New restaurant submited. Please wait for approval from the admins.</p>
            <p style="float: right;"><a class="btn" href="submitentry4.php">Submit another restaurant »</a></p>
            <?php
				} else {
					echo "Please log in.";
				}
