        ...
        <div style="display: inline-block; text-align: left; padding: 30px; background-color: #eeeeee; border-radius: 6px;">
            ...
            <!-- restaurant info -->
			  <?php
                ...
                $resultResturant = mysql_query("SELECT * FROM restaurant WHERE stall_name ='$stall_name' AND approve = 1");
                
                echo "<h1 style='color: black; margin-top: 0;'>$stall_name</h1>";
                ...
                $rownum = 1;
                while($row1 = mysql_fetch_array($resultResturant))
					{
					echo "<tr>";
					...
					$resultRatings = mysql_query("SELECT ROUND(AVG(rating),2) AS avgRatings FROM review WHERE r_stall_name ='$stall_name' AND r_address = '$address'");
					
					while($row2 = mysql_fetch_array($resultRatings))
					{
					   echo "<td>" . $row2['avgRatings'] . "</td>";
					}
				//	echo "<td><a href='submitreview.php?stallnamess=" . $stall_name . "&addresses=" . $address."'>New review</a></td>";
					echo "<td><a href='viewResturantReview.php?stallnamess=" . $stall_name . "&addresses=" . $address."'>View reviews</a></td>";
